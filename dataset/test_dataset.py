...
class SceneGaussianDataset(Dataset):
    def __init__(self, scene_model_dir, scene_source_dir, model_args, iterations = 300000, split = "test"):
        ...
        # self.scene_model_dir = scene_model_dir
        # self.scene_source_dir = scene_source_dir
        ...
        if split == "train":
            ...
            self.transformsfile = "transforms_test.json"
        else:
            self.transformsfile = "transforms_all.json"
        # self.transformsfile = "transforms_train.json" if split == "train" else "transforms_test.json"
    ...
    def load_viewpoint(self, path):
        ...
        cam_infos = []
        for viewpoint_idx in range(frames_len):
            ...
            FovY = fovy 
            FovX = fovx
            cam_infos.append(CameraInfoDpt(uid=viewpoint_idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, ray = ray,
    ...
    def __getitem__(self, scene_idx):
        ...
        cam_infos = self.load_viewpoint(source_path)
        camera_list = []
        for id, c in enumerate(cam_infos):
            camera_list.append(loadCam(self.gaussian_args, id, c, resolution_scale=1.0))
        viewpoint = camera_list[0]
        ...
        return gaussians, camera_list, view_rays, model_path
...
class ScenePointCloudDataset(Dataset):
    def __init__(self, scene_model_dir, scene_source_dir, model_args, iterations = 300000, split = "test"):
        ...
        if split == "train":
            ...
            self.transformsfile = "transforms_all.json"
        # self.transformsfile = "transforms_train.json" if split == "train" elif "transforms_test.json" 
        self.iterations = iterations
        self.length = len(self.scene_model_dir)
        print("Training scenes", self.length)
