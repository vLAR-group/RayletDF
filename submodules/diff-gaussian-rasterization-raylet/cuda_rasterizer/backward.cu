	...
	float* dL_dcov)
{
	...
	if (denom2inv != 0)
	{
		...
		// e.g., dL / da = dL / d_conic_a * d_conic_a / d_a 矩阵对矩阵求导，每个位置对对应位置求导后加权和
	...
	glm::vec4* dL_drot)
{
	...
	// dL_dmean = dmean(ndc)_dmean * d_mean2D_dmean(ndc) * dL_dmean2D
	// dmean(ndc)_dmean = P^T  
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Propagate gradients from pixel features to features
			if (geo_feats){
