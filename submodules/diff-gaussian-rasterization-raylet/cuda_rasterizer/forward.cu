	...
	bool prefiltered)
{
	...
	float4 p_hom = transformPoint4x4(p_orig, projmatrix); //此处projmatrix为全投影矩阵，包含viewmatrix
	...
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv }; // inverse 2D covariance
	...
	float lambda1 = mid + sqrt(max(0.1f, mid * mid - det)); // 求解特征值|Cov - λI| = 0, 二次方程求根公式
	float lambda2 = mid - sqrt(max(0.1f, mid * mid - det));
	float my_radius = ceil(3.f * sqrt(max(lambda1, lambda2))); // https://blog.csdn.net/qaqwqaqwq/article/details/136886008
	float2 point_image = { ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H) }; //[-1, 1] to [W, H]
	uint2 rect_min, rect_max;
	getRect(point_image, my_radius, rect_min, rect_max, grid); //计算覆盖的tiles
	...
	const int topk)
{
	...
	float D = 0;
	// float weight_max=0;
	// float second_weight_dpt=0;
	// float second_weight=0;

	float topk_weight[MAX_TOPK]= { 0 };
	int topk_idx[MAX_TOPK]= { 0 };
	float min_heap = 0.0;
	int min_heap_idx = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// 		F[ch] += geo_feats[collected_id[j] * NUM_FEATURE_CHANNELS + ch] * alpha_T; 
			// 	}
			// }
			weight += alpha_T;
			// D += depths[collected_id[j]] * alpha_T;
			Minimum(topk_weight, min_heap, min_heap_idx, topk);
			if (min_heap < alpha_T){
				topk_weight[min_heap_idx] = alpha_T;
				topk_idx[min_heap_idx] = collected_id[j];
			}
	...
	if (inside)
	{
		...
		// if (geo_feats){
		// 	for (int ch = 0; ch < NUM_FEATURE_CHANNELS; ch++)                 
		// 		out_feat[ch * H * W + pix_id] = F[ch];
		// }
		out_alpha[pix_id] = weight; //1 - T;
		for (int ch = 0; ch < topk; ch++)                 
			out_depth[ch * H * W + pix_id] = topk_idx[ch];
...
// Perform initial steps for each Gaussian prior to rasterization.
template<int C>
__global__ void pointcloud_preprocessCUDA(int P, int D, int M,
	const float* orig_points,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	bool prefiltered)
{
	...
	float lambda2 = mid - sqrt(max(0.1f, mid * mid - det));
	float my_radius = ceil(1.f * sqrt(max(lambda1, lambda2)));
	float2 point_image = { ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H) }; //[-1, 1] to [W, H]
	uint2 rect_min, rect_max;
	...
	const int topk)
{
	...
	float topk_weight[MAX_TOPK]= { 0 };
	int topk_idx[MAX_TOPK]= { -1 };
	for (int ch = 0; ch < MAX_TOPK; ch++){
		topk_weight[ch] = -10.0;
		topk_idx[ch] = -1;
	}
	float min_heap = 0.0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// PT_Minimum(topk_weight, min_heap, min_heap_idx);
			// if (min_heap < alpha){
			// 	topk_weight[min_heap_idx] = alpha;
			// 	topk_idx[min_heap_idx] = collected_id[j];
			// }
			...
			float foot_distance = -1.0f * sqrt(min_value / (view2gaussian_j[0]));
			// float foot_distance = alpha;
			Minimum(topk_weight, min_heap, min_heap_idx, topk);
			if (min_heap < foot_distance){
				topk_weight[min_heap_idx] = foot_distance;
				topk_idx[min_heap_idx] = collected_id[j];
			}
...
void FORWARD::pointcloud_render(
	const dim3 grid, dim3 block,
	const uint2* ranges,
	const uint32_t* point_list,
	int W, int H,
	...
	const int topk)
{
	pointcloud_renderCUDA<NUM_CHANNELS> << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
...
void FORWARD::pointcloud_preprocess(int P, int D, int M,
	const float* means3D,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	bool prefiltered)
{
	pointcloud_preprocessCUDA<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		means3D,
		scales,
		scale_modifier,
