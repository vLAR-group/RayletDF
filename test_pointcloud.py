...
from dataset.test_dataset import ScenePointCloudDataset, collate_fn
from utils.ray_utils import get_surface_normal
...
def test(dataset, pipe, args):
    ...
    gs_dataset = ScenePointCloudDataset(args.scene_model_dir, args.scene_source_dir, dataset, iterations=args.iterations, split=args.split)
    ...
    for i, (gaussians, viewpoint_cam_list, view_rays, model_path) in enumerate(test_datloader):
        ...
        for idx, viewpoint_cam in enumerate(viewpoint_cam_list):
            ...
            for i in range(0, iter_num):
                ...
                batch_valid_idx = valid_idx[i*iter_step:(i+1)*iter_step]
                if args.normal:
            ...
            if args.filter_outlier:
                valid_mask = (render_idx > -1).sum(-1) > 0
                mask = (viewpoint_cam.dist > 0.1) * valid_mask.cpu().numpy()
            else:
                mask = viewpoint_cam.dist > 0.1
            ade, rmse, rel_ade, sq_rel_ade, a1, a2, a3 = complete_ray_metric(out_dist, viewpoint_cam.dist, mask=mask)
            ades.append(ade) 
            rmses.append(rmse)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filter_outlier", type=bool, default=False)
