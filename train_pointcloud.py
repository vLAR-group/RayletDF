...
from gaussian_renderer import render_pointcloud_raylet
from utils.general_utils import safe_state, build_invcovariance_from_scaling_rotation, inverse_sigmoid
...
from dataset.train_dataset import ScenePointCloudDataset, collate_fn
...
def train(dataset, pipe, args):
    ...
    if distributed:
        ...
        gs_dataset = ScenePointCloudDataset(args.scene_model_dir, args.scene_source_dir, dataset, iterations=args.iterations)
        ...
        # args.iterations = int(args.iterations / dist.get_world_size())
    ...
    for i, (gaussians, viewpoint_cam, view_rays) in enumerate(train_datloader):
        ...
        dist2 = torch.clamp_min(distCUDA2(gaussians.get_xyz.cuda()), 0.0025)
        gs_scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        opacities = inverse_sigmoid(0.99*torch.ones((gaussians.get_xyz.shape[0], 1), dtype=torch.float, device=device))
        gaussians._opacity = opacities
        gaussians._scaling = gs_scales
        ...
        cur_iteration += 1
        with torch.no_grad():
            render_pkg = render_pointcloud_raylet(viewpoint_cam, gaussians, pipe, background, topk=TOPK)
            gt_dist = torch.from_numpy(viewpoint_cam.dist).to(device).flatten()
            mask = gt_dist > 0.1
            render_idx = render_pkg["depth"].long().permute(1,2,0)
            if args.filter_outlier:
                valid_mask = ((render_idx > -1).sum(-1) > 0).flatten()
                mask = mask * valid_mask
            valid_idx = render_idx.reshape(-1, TOPK) > -1
        ...
        if TOPK == 1:
            pred_dist = dist_delta+t_opt.squeeze()
        else:
            unc = render_pkg_net[...,1]
            unc[~valid_idx] = -10.0
            unc = torch.softmax(unc, dim=-1)
            pred_dist = ((dist_delta+t_opt.squeeze())*unc).sum(-1)
        loss = l1_loss_mask(pred_dist, gt_dist[bs_ind], mask=mask[bs_ind])   
        loss.backward() 
        optimizer.step()
        ...
        if (cur_iteration in args.save_iterations or (cur_iteration % 10000 == 0)) and (local_rank == 0):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dpt_lr", default=1e-4, type=float, help="Learning rate")
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[5000, 7000, 10_000, 20_000, 30_000, 40_000, 50_000, 60_000])
    parser.add_argument("--testing_iterations", nargs="+", type=int, default=[7_000, 10_000, 20_000, 30_000, 40_000, 50_000, 60_000])
    parser.add_argument("--filter_outlier", type=bool, default=False, help="Whether to filter out outlier rays that do not intersect with any point ball")
